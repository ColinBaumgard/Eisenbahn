...
fn track_tool_ghost_system(
    ...
    mut q_nodes: Query<
        ...
        With<NodeComp>,
    >,
    mut q_active_ghost_track: Query<(Entity, &mut EdgeComp), (With<ActiveGhost>, With<EdgeComp>)>,
    ...
    if !ev_left.is_empty() {
        ...
        let mut hook = None;
        let mut active_ghost_node = None;
        let active_ghost_edge = q_active_ghost_track.get_single();
        for (e, t, is_selected, is_active_ghost) in q_nodes.iter() {
            let d = t.translation.truncate().distance(mouse.position);
            if d < 10.0 {
                hook = Some((e, t.translation.truncate()));
            }
            if is_selected.is_some() {
                selected_nodes.push((e, t.translation.truncate()));
            }
            if is_active_ghost.is_some() {
                active_ghost_node = Some(e);
            }
        }
        if selected_nodes.len() == 0 && hook.is_none() {
            ...
            let mut e_node;
            let mut pos_b = mouse.position;
            if hook.is_none() {
                let mut node = get_node_bundle(mouse.position);
                e_node = commands.spawn(node).id();
            } else {
                (e_node, pos_b) = hook.unwrap();
            }

            let mut track =
                get_track_bundle(selected_nodes[0].0, e_node, &selected_nodes[0].1, &pos_b);
            let e_track = commands.spawn(track).id();
            ev_ghost.send(GhostEvent::add(vec![e_node, e_track]))
        }
    } else {
        for (e, mut t, is_selected, is_active_ghost) in q_nodes.iter_mut() {
            if is_active_ghost.is_some() {
                t.translation.x = mouse.position.x;
                t.translation.y = mouse.position.y;
            }
        }
        let active_ghost_edge = q_active_ghost_track.get_single_mut();
        if active_ghost_edge.is_ok() {
            let (e, mut edge) = active_ghost_edge.unwrap();
            edge.pos_b = mouse.position;
            commands.entity(e).insert(Update);
        }
    }
    ...
    if keys.just_pressed(KeyCode::Return) {
        ev_ghost.send(GhostEvent::deghost(Vec::new()));
    }
...
fn track_tool_selection_system(
    ...
    mut ev_ghost: EventWriter<GhostEvent>,
    keys: Res<Input<KeyCode>>,
    q_active_ghost: Query<Entity, With<ActiveGhost>>,
    q_nodes: Query<(Entity, &Transform), With<NodeComp>>,
) {
    if !q_active_ghost.is_empty() {
        return;
    }
    if !ev_left.is_empty() {
        for (e, t) in q_nodes.iter() {
            ...
            if d < 10.0 {
                ...
                let mut node = get_node_bundle(mouse.position);
                e_node = commands.spawn(node).insert(ActiveGhost).id();

                let mut track = get_track_bundle(e, e_node, &t.translation.truncate(), &pos_b);
                let e_track = commands.spawn(track).insert(ActiveGhost).id();
                ev_ghost.send(GhostEvent::add(vec![e_node, e_track]));
                return;
            }
        }
...
fn update_ghost_system(
    ...
    for ev in g_events.iter() {
        match ev.action {
            GhostAction::Add => {
                for (e, mut stroke, n_c, g_c, is_ghosted) in q_all.iter_mut() {
