    ...
    fn build(&self, app: &mut App) {
        ...
        app.add_systems((
            enter_system
                ...
                .run_if(resource_equals(Tool::TrackEditor)),
            exit_system
                .run_if(resource_changed::<Tool>())
                .run_if(not(resource_equals(Tool::TrackEditor))),
        ));

        app.add_systems((
            update_hooked_on_cursor_system.run_if(resource_equals(Tool::TrackEditor)),
            update_tracks_system
                .after(update_hooked_on_cursor_system)
                .run_if(resource_equals(Tool::TrackEditor)),
            ...
            track_editor_building_system
                ...
                .after(update_tracks_system)
                .run_if(resource_equals(Tool::TrackEditor)),
            position_hook_system
                .after(track_editor_building_system)
                .run_if(resource_equals(Tool::TrackEditor)),
        ));
    }
}
...
fn enter_system(mut commands: Commands, q_nodes: Query<Entity, (With<NodeComp>)>) {
...
fn exit_system(
    ...
    q_to_be_deleted: Query<Entity, Or<(With<HookedOnCursor>, With<AttractionRing>)>>,
    q_to_be_deselected: Query<Entity, With<Selected>>,
    q_building: Query<Entity, With<Building>>,
    mut graph: ResMut<TrackGraph>,
) {
    for e in q_to_be_deselected.iter() {
        commands.entity(e).remove::<Selected>();
    }
    for e in q_to_be_deleted.iter() {
